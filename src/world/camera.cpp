...
const float4x4 cg::world::camera::get_view_matrix() const
{
	...
	return float4x4({
		...
		{ -dot(x_axis, get_position()), -dot(y_axis, get_position()), -dot(z_axis, get_position()), 1 }	
	});
}
...
const DirectX::XMMATRIX cg::world::camera::get_dxm_view_matrix() const
{
	...
	DirectX::FXMVECTOR up_direction{0.f, 1.0f, 0.f};
...
{
	return cross(get_direction(), float3{0.f, 1.0, 0.f});
}
